...
```{r, message=FALSE, comment=""}
# 図5(1)　細胞組織／柔細胞の断片の含有状況
ggplot(tissue, aes(x = 西暦, y = 細胞組織)) +
  geom_line() +
  scale_x_continuous(breaks = c(1150,1200,1300,1400,1500,1600,1700,1800,1900), 
                     limits = c(1150,1860), expand = c(0,0)) + # X軸の目盛りを指定
  scale_y_continuous(breaks = c(0,100,200,300,400,500,600), limits = c(0,600), expand = c(0,0))+ # Y軸の目盛りを指定
  labs(x = "年", y = "細胞組織（片）") +
  theme_classic()
ggsave(file = "fig5-1.png", dpi = 300) # ファイルの保存
```
...
ggplot(tissue, aes(x = 西暦, y = 繊維)) +
  ...
  labs(x = "年", y = "繊維（片）") +
  theme_classic()
ggsave(file = "fig5-2.png", dpi = 300) # ファイルの保存
```
